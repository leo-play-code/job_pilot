...
12. **Stripe 月費訂閱**：$19.99/月；Free 用戶每日 10 次 AI 限制，Pro 用戶無限；訂閱流程：Stripe Hosted Checkout → webhook 更新 plan → Stripe Customer Portal 管理帳單；`/pricing` 方案比較頁 + `/settings/billing` 帳單管理頁
13. **Admin 全資料庫管理介面**：`/admin/dashboard` 新增 4 個 tab（使用者 / 履歷 / 自薦信 / 使用記錄）；可瀏覽所有用戶的 Resume、CoverLetter、UsageLog；Resume 和 CoverLetter 可搜尋與刪除；UsageLog 唯讀可篩選；Account/Session/VerificationToken（NextAuth 內部表）不對外暴露；**使用者 Tab 新增刪除功能**（cascade 刪除全部資料）+ 分頁（10/page）；不允許刪除 admin 自己
14. **全站分頁系統（Pagination）**：Dashboard 履歷/自薦信列表、Resume Builder 模板選擇器、Admin 三個資料 Tab 均採每頁 6 筆分頁；共用 `Pagination` 元件（Smart Ellipsis：永遠顯示第 1 頁和最後一頁，中間省略時顯示 "..."）；Edit Mode 下 Pagination 隱藏以便批量操作；`GET /api/resume`、`GET /api/cover-letter`、`GET /api/templates` 加 `?page&limit` 分頁參數
15. **行政區全選**：104 自動投遞 `SearchConfigForm` 的行政區 checkbox 列頂端新增「全部區」checkbox；勾選自動全選該縣市所有行政區；部分選中時顯示 indeterminate 狀態；手動全選後自動打勾
...
- [ ] PDF 下載內容與預覽一致
- [ ] 有圖片的頁面使用 `BlurImage` 元件（模糊→清晰漸進載入）
- [ ] `next.config.ts` 允許 Supabase storage 域名，圖片不報 hostname 錯誤
- [ ] 所有頁面切換顯示頂部進度條（nextjs-toploader）
- [ ] 所有異步按鈕操作有 loading spinner + disabled 狀態
- [ ] 全站使用 sonner 統一 toast 通知，無 inline toast state
