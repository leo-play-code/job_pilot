...
13. **Admin 全資料庫管理介面**：`/admin/dashboard` 新增 4 個 tab（使用者 / 履歷 / 自薦信 / 使用記錄）；可瀏覽所有用戶的 Resume、CoverLetter、UsageLog；Resume 和 CoverLetter 可搜尋與刪除；UsageLog 唯讀可篩選；Account/Session/VerificationToken（NextAuth 內部表）不對外暴露
